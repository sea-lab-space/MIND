...
from generate_mock_data.rpa.note_writer import mock_progress_note
...
You are given a depression |patient's persona| and the |first mental health therapy session transcript| of the patient.
...
2) The encounters should include the date, type of encounter, medical condition, ICD-10-CM code, and CPT code.
3) The encounters should be generated based on the |patient's persona| and the |first mental health therapy session transcript|.
...
7) There should be at least 1 Hospital Encounter.
"""
...
if __name__ == '__main__':
    ...
    for patient, persona in tqdm(data.items()):
        ...
        encounter_prompt = f'''
            ...
            |First mental health therapy session transcript|:
            {transcript}
        '''
        ...
        # write clinical notes based on encounters
        # use concurrent
        notes = asyncio.run(mock_progress_note(encounters, persona, MODEL_NAME))
        assert len(encounters) == len(notes)
        for i, note in enumerate(notes):
            encounters[i]['notes'] = note
